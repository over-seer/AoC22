...
bool is_list(const string & str){return str.front()=='[' && str.back()==']';}
...
int compare_list(const string & str1, const string & str2);
...
int compare_list(const string & list1, const string & list2)
{
  int rtn=0;
...
void part2(string fn)
{
  ...
  auto sort_rule = [] (const string & s1, const string & s2) -> bool
  {
    return compare_list(s1,s2)==-1;
  };

  sort(packets.begin(),packets.end(),sort_rule);
  ...
  cout << fn << " Answer to part 2 = " << product << endl;
}
